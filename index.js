...
async function createWhatsAppConnection(chatId, phoneToPair = null) {
    ...
    const waSock = makeWASocket({
        ...
        // Canonical browser identity for phone-number pairing.
        browser: Browsers.ubuntu('Chrome'),

        // Railway does not need a terminal QR for this pairing flow.
        ...
        // More tolerant for a cloud/Railway connection.
    ...
    async function requestPairingCodeWhenReady() {
        ...
        try {
            ...
            if (!/^\d{8,15}$/.test(cleanPhone)) {
                throw new Error('Invalid phone number. Use country code + number, digits only.');
            }

            console.log(`[${chatId}] Requesting WhatsApp pairing code...`);
            ...
            await bot.sendMessage(
                chatId,
                `🔑 **Pairing Code:** \`${code}\`\n\n👉 WhatsApp → **Linked Devices** → **Link a Device** → **Link with phone number instead** → code-ti din.\n\n⏳ Code-ti expire howar age use korun.`,
                {
                    parse_mode: "Markdown",
                    reply_markup: {
                        inline_keyboard: [
            ...
            console.log(`[${chatId}] Pairing code sent.`);
        } catch (err) {
            codeRequested = false;
            console.error(`[${chatId}] Pairing Request Error:`, err);

            await bot.sendMessage(
                chatId,
                "❌ Pairing code generate hoyni. Ektu pore abar try koren.",
    ...
    waSock.ev.on('connection.update', async (update) => {
        ...
        // Wait for Baileys' ready/QR stage before requesting the code.
        if (
            phoneToPair &&
            !codeRequested &&
            !waSock.authState.creds.registered &&
