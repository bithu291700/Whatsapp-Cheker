...
async function createWhatsAppConnection(chatId, phoneToPair = null) {
    ...
    waSock.ev.on('connection.update', async (update) => {
        ...
        // Pairing Code mode does not need to wait for a QR event.
        // Request the code once the socket has started initializing.
        if (phoneToPair && !codeRequested && !state.creds.registered &&
            ...
            try {
                ...
                console.error("Pairing Request Error:", err?.stack || err);
                codeRequested = false;

                await bot.sendMessage(
                    chatId,
                    "❌ Pairing code generate hoyni. Number/check connection kore abar try koren.",
...
bot.on('message', async (msg) => {
    ...
    if (text === "📱 Check Number") {
        ...
        const isConnected = waSock && (waSock.user || userAuthStates[chatId]?.creds?.me || userAuthStates[chatId]?.creds?.registered);
    ...
    if (text === "ℹ️ Help & Status") {
        const waSock = userSockets[chatId];
        const isConnected = waSock && (waSock.user || userAuthStates[chatId]?.creds?.me || userAuthStates[chatId]?.creds?.registered) ? "✅ Connected" : "❌ Not Connected";
